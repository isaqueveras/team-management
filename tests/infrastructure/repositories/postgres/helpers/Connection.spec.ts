...
import { PGConnection, ConnectionNotFoundError } from '@/infrastructure/repositories/postgres/helpers'
...
describe('PGConnection', () => {
  ...
  it('should get repository', async () => {
    await sut.connect()
    await sut.commit()
    const repository = sut.getRepository(PGTeam)
    expect(getRepositorySpy).toHaveBeenCalledWith(PGTeam)
  ...
  it('should return ConnectionNotFoundError on getRepository if connection is not found', async () => {
    ...
    expect(() => sut.getRepository(PGTeam)).toThrow(new ConnectionNotFoundError())
  })
})
